...
function setUserInfoCookie(key, value) {
  ...
  const expirationDate = new Date(Date.now() + 10 * 1000); // Додаємо 10 секунд до поточного часу

  // Формуємо cookie зі значенням та датою закінчення
  document.cookie = `userInfo=${encodedValue}; expires=${expirationDate.toUTCString()}; path=/`;

  console.log(`Cookie userInfo встановлено: ${key} = ${value}`);
}
...
// Зберігає інформацію про користувача в sessionStorage
function saveUserInfo(key, value) {
  sessionStorage.setItem(key, value); // Зберігає пару ключ-значення
  console.log(`Saved ${key}: ${value}`); // Виводить повідомлення у консоль
}

// Отримує інформацію про користувача з sessionStorage
function getUserInfo(key) {
  const value = sessionStorage.getItem(key); // Отримує значення за ключем
  if (value !== null) {
    console.log(`Retrieved ${key}: ${value}`); // Виводить повідомлення у консоль
    return value; // Повертає отримане значення
  } else {
    console.log(`No value found for key: ${key}`); // Повідомлення, якщо значення не знайдено
    return null; // Повертає null, якщо значення не знайдено
  }
}
